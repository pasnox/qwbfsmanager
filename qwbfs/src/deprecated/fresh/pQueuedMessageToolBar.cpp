...
void pQueuedMessageToolBar::paintEvent( QPaintEvent* event )
{
	...
	painter.setBrush( pal.color( backgroundRole() ).darker( 110 ) );
	painter.drawRoundedRect( contentsRect(), 8, 8 );
	
	painter.setPen( Qt::NoPen );
	painter.setBrush( pal.brush( backgroundRole() ) );
	painter.drawRoundedRect( contentsRect().adjusted( 1, 1, -1, -1 ), 8, 8 );
}
