...
ListViewDelegate::ListViewDelegate( ListViewV2* view, AbstractFileSystem* model, pNetworkAccessManager* cache )
...
void ListViewDelegate::paintList( QPainter* painter, const QStyleOptionViewItemV4& option, const QModelIndex& index ) const
{
	...
	// title/region
	{
		...
		QString text;// = QString( "%1 - %2 (%3 - %4)" ).arg( entry.id() ).arg( pCoreUtils::toTitleCase( entry.title() ) ).arg( QWBFS::Driver::regionToString( entry.region() ) ).arg( QWBFS::Driver::regionToLanguageString( entry.region() ) );
	...
	// size/origin
	{
		...
		QString text = tr( "Estimated size: %1 - FilePath: %2" ).arg( pCoreUtils::fileSizeToString( entry.size() ) ).arg( entry.filePath() );
		text = metrics.elidedText( text, Qt::ElideRight, rect.width() );
...
void ListViewDelegate::paintIcon( QPainter* painter, const QStyleOptionViewItemV4& option, const QModelIndex& index ) const
{
	...
	QPixmap cover;// = mModel->coverPixmap( entry.id(), rect.size() -QSize( 0, painter->font().pointSize() +spacing ) );
	QPixmap state;// = mModel->statePixmap( entry.state(), QSize( 24, 24 ) );
	...
	// state
	/*if ( !state.isNull() && entry.state() != QWBFS::Driver::None ) {
